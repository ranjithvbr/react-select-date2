...
function CalenderField(props) {
    ...
    const startSetError = (e) => {
            ...
            if ((e.code === 'Enter' && startDate) || (e._reactName === "onBlur" && startDate)) {
                ...
                if(new Date(startDate).getFullYear() >= new Date(maxDate).getFullYear()){
                    setErrMsg_start(`year must be ${new Date(maxDate).getFullYear()} or earlier`)
                }else if(new Date(startDate).getFullYear() <= new Date(minDate).getFullYear()){
                    setErrMsg_start(`year must be ${new Date(minDate).getFullYear()}  or later`)
    ...
    const endSetError = (e) => {
        ...
        if ((e.code === 'Enter' && endDate) || (e._reactName === "onBlur" && endDate)) {
            const {minDate, maxDate} = minAndmaxDate;
            if(new Date(endDate).getFullYear() >= new Date(maxDate).getFullYear()){
                setErrMsg_end(`year must be ${new Date(maxDate).getFullYear()} or earlier`)
            }else if(new Date(endDate).getFullYear() <= new Date(minDate).getFullYear()){
                setErrMsg_end(`year must be ${new Date(minDate).getFullYear()}  or later`)
            }else if(new Date(startDate) >= new Date(endDate)){
    ...
    console.log(minAndmaxDate.minDate, minAndmaxDate.maxDate, "selectedDateFromField" )
